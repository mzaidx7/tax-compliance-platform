...
- Full client-master CSV downloads now require recent password confirmation before the file is streamed and before the download audit is written. This protects the export containing passport, Emirates ID and trade-licence values while preserving the administrator or assigned-staff permission boundary.
- Release documentation now reflects the approved calculated VAT and Corporate Tax scheduling boundary. The export remains CSV for the MVP; a formatted workbook export is a future usability enhancement, not a release blocker.
- Final verification after the export protection: all bounded PHPUnit groups passed for 479 tests and 1,515 assertions; focused export tests passed with 11 tests and 52 assertions; Pint, PHPStan and the Vite production build passed. Composer reported no locked dependency advisories and npm reported zero production vulnerabilities.
- `platform:release-check --json` correctly remains incomplete in the local environment: production environment, debug, HTTPS URL, encrypted and secure session cookies, external mail transport, and the two MVP feature flags require a real production `.env` and host. Local browser QA on both themes is recorded as completed on 28 July; production deployment and post-deployment smoke checks remain outstanding.
