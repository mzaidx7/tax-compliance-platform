...
- Last updated: 2026-07-28 02:40 +04:00
- Updated by: Claude Sonnet 5 (Claude Code)
- Current parent repository branch: `main`
- Current parent baseline commit: `1071185`
- Platform repository status: not yet initialised as a separate repository
- Parent repository status for this directory: entire `TBT Compliance Platform/` directory is untracked
- Current phase: Stage 3, compliance walking skeleton
- Current milestone: Operational notification triggers packet completed
...
The immediate objective is an authorized, audited download route for stored export artifacts.
...
- PHPUnit passed 320 tests with 985 assertions.
...
1. Add an authorized, audited download route for stored export artifacts.
2. Add a follow-up aware work register view that groups an obligation's primary work and its follow-ups.
3. Add a firm dashboard summarising due work, high risk and overdue payments from existing records.
...
- Implemented and verified the audit register export (`AuditRegisterFilters`, `ExportAuditRegister`) reusing the bounded spreadsheet-safe CSV writer and tenant-private storage, gated by the `audit_viewer` flag and the named `view_audit_log` permission, streaming filtered records in chunks, exporting no before or after values so redacted data can never be restored, and recording each download as its own `audit_register.exported` action with file name, checksum, row count, bytes and filter metadata but never the search term. The viewer and export share one filter object so they can never disagree. No browser download route exists yet.
